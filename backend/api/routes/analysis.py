...
async def get_analysis_status(analysis_id: str):
    """Get the current status of an analysis including progress."""
    async with get_db() as db:
        cursor = await db.execute(
            "SELECT id, document_name, status, overall_score, overall_level, progress_pct, progress_step, created_at, completed_at FROM analyses WHERE id = ?",
            (analysis_id,),
        )
        row = await cursor.fetchone()
        if not row:
